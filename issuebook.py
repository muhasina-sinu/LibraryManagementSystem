...
def issue():
    ...
    try:
        ...
        check2 = "select EXISTS(select * from books where book_id = %s);"% (temp1)
        cur.execute(check)
        result = cur.fetchone()
        cur.close()
        cur = con.cursor()
        ...
        if temp2 =='' or result[0] == 1 or result2[0] == 0:
        ...
        messagebox.showerror("Error", "Please check the input values")
